%Author: Michael Busa
%Homework #2: Q1 part 1
%Date: 2/24/2020
%Purpose: Import validation set, find besst performance metrics, use 10,
%         100, and 1000 samples to train linear classifier, test against
%         validation set, find error
...
t = GD(z,N,true_class_labels,t_init,1,1000);
...
% Prepare figure for boundary
horizontalGrid = linspace(floor(min(samples(:,1))),ceil(max(samples(:,1))),101);
verticalGrid = linspace(floor(min(samples(:,2))-4),ceil(max(samples(:,2))+1),91);
[h,v] = meshgrid(horizontalGrid,verticalGrid);
z_test = [ones((91*101),1) h(:) v(:)];
...
% Prepare figure for boundary
horizontalGrid = linspace(floor(min(samples(:,1))),ceil(max(samples(:,1))),101);
verticalGrid = linspace(floor(min(samples(:,2))-2),ceil(max(samples(:,2))+1),91);
[h,v] = meshgrid(horizontalGrid,verticalGrid);
z_test = [ones((91*101),1) h(:) v(:)];
sigmoid = 1./(1+exp(-z_test*t));
min_vals = min(sigmoid);max_vals = max(sigmoid);
...
z_valid = [ones(1,N); samples(1,:); samples(2,:)]';
...
axis equal,
%% Functions
function t = GD(samples, N, true_class_labels, t, alpha, runs)
    c = zeros(runs, 1);
    for i = 1:runs % while norm(cost_gradient) > threshold
        h = 1 ./ (1 + exp(-samples*t));	% Sigmoid function   
        c(i) = (-1/N)*((sum(true_class_labels' * log(h)))+(sum((1-true_class_labels)' * log(1-h))));
        c_gradient = (1/N)*(samples' * (h - true_class_labels));
